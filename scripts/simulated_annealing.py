...
class SimulatedAnnealing:
    ...
    def make_n_random_edits(self, seq, nedits, min_pos=None, max_pos=None):
        ...
        for i in range(nedits):
            # random mutation
            pos = pos_to_mutate[i]
            # print(seq.shape, seq[pos].shape, seq[pos])
            cur_AA = seq[pos].item()
    ...
    def run(self, initial_population, num_steps, oracle, args, logger, save_path, min_pos=None, max_pos=None, log_every=50):
         with torch.no_grad():
            ...
            for i in tqdm(range(num_steps), desc='simulated annealing'):
                ...
                # print(f'x_proposal shape: {x_proposal.shape}')
                proposal_energy = run_predictor(oracle, x_proposal, args).detach().cpu()
                # print(f'proposal_energy shape: {proposal_energy.shape}')
                # Make sequence acceptance/rejection decisions
                aprob = self.acceptance_prob(proposal_energy, current_energy)
                # print(f'aprob shape: {aprob.shape}')                
                aprob = (aprob > torch.rand_like(aprob)).float()
                aprob = aprob.unsqueeze(1)
                x_new = x_proposal * aprob + x * (1. - aprob)
                # print(f'aprob: {aprob.shape}')
                # print(f'x_proposal shape: {x_proposal.shape}')
...
if __name__ == '__main__':
    ...
    logger = common.get_logger('simulated_annealing', log_dir)
    logger.info(args)
    logger.info(config)
    shutil.copyfile(args.config, os.path.join(log_dir, os.path.basename(args.config)))
    
    oracle = load_oracle(config, args, logger)
    sampler = SimulatedAnnealing(config)

    initial_seqs = load_seqs_from_csv(csv_path=config.base_pool_path, 
                                      topk=config.base_pool_topk,
                                      label_column=config.base_pool_label_column)
