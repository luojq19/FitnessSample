...
def main():
    ...
    if config.evmutation_dist_file is not None:
        from scipy.sparse import load_npz
        knn_graph = load_npz(config.evmutation_dist_file)
        logger.info(f'Loaded kNN graph from {config.evmutation_dist_file}')
    else:
        knn_graph = kneighbors_graph(
            all_seqs_list, n_neighbors=500, metric='l1', mode='distance',
            include_self=True, n_jobs=20)
        print(knn_graph.shape, type(knn_graph))
        print(f'saving the knn_graph')
        from scipy.sparse import save_npz
        save_npz('data/evmutation/knn_graph_edit_dist.npz', knn_graph)
        print(f'saved the knn_graph')
        input()
