...
from torchmetrics.functional import spearman_corrcoef, pearson_corrcoef, matthews_corrcoef, mean_squared_error, mean_absolute_error
import numpy as np
import json, os, argparse, time, shutil
...
def train(model, train_loader, val_loader, criterion, optimizer, scheduler, device, config, logger, writer):
    model.train()
    n_bad = 0
    best_val_loss = 1.e10
    best_pearson = 0
    ...
    for epoch in range(config.train.num_epochs):
        ...
        if val_pearson < best_pearson + epsilon and val_spearman < best_spearman + epsilon:
            ...
            logger.info(f'New best performance found! pearson={val_pearson:.4f}; spearman={val_spearman:.4f}')
            n_bad = 0
...
def main():
    ...
    # datasets
    all_data = FitnessDataset(config.data.data_file, config.data.label_column)
    ...
    if args.gen:
        ...
        outputs = []
        labels = []
        ckpt = torch.load('logs/train_predictor_stability_2023_09_07__18_16_15_debug/checkpoints/best_checkpoints.pt')
        model.load_state_dict(ckpt)
        for data, label in train_loader:
            data, label = data.to(args.device), label.to(args.device)
            output = model(data)
            outputs.append(common.toCPU(output))
            labels.append(common.toCPU(label))
        ...
        # Plot the KDE curves
        sns.kdeplot(outputs, label='predictions', color='blue')
        sns.kdeplot(labels, label='ground truth', color='green')

        # Add title and labels
        plt.title('Distributions of predictions vs. ground truth')
        plt.xlabel('Value')
        plt.ylabel('Density')
    ...
    train(model, train_loader, val_loader, criterion, optimizer, scheduler, args.device, config, logger, writer)
