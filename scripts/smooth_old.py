...
@hydra.main2(version_base="1.3", config_path="../configs", config_name="GS.yaml")
def main2(cfg: DictConfig) -> Optional[float]:
    logging.basicConfig()
    logging.root.setLevel(logging.NOTSET)
    logger = logging.getLogger('Graph-based Smoothing')
    pyrootutils.setup_root(__file__, indicator=".project-root", pythonpath=True)
...
def main():
    ...
    predictor = globals()[predictor_config.model]
