...
def evaluate(sampled_seqs, gt_seq2label, save_dir=None, tag=None):
    ...
    fit_E3_normalized = [normalize_fit_E3(fit_E3[i]) for i in range(len(fit_E3))]
    fit_E2_normalized = [normalize_fit_E2(fit_E2[i]) for i in range(len(fit_E2))]
    ...
    metrics_df = pd.DataFrame({'metric': ['fit_E3', 'fit_E2', 'fit_E3_normalized', 'fit_E2_normalized'],
